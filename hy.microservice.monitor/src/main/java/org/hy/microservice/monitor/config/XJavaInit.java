package org.hy.microservice.monitor.config;
...
public class XJavaInit extends AppInitConfig
{
    ...
    private synchronized void init(boolean i_IsStartJobs)
    {
        if ( !$Init )
        {
            ...
            try
            {
                this.init("config/monitor/ms.monitor.sys.Config.xml"     ,this.xmlRoot);
                this.init("config/monitor/ms.monitor.startup.Config.xml" ,this.xmlRoot);
                this.init((List<Param>)XJava.getObject("StartupConfig_MS_Monitor") ,this.xmlRoot);
                this.init(((Param)XJava.getObject("MS_Monitor_RootPackageName")).getValue());
            }
